...
Seguindo as idéias do parágrafo anterior, mas agora considerando o grafo $G/X$ resultante da contração do conjunto $X$ ao vértice $x$, % obtemos uma coleção de $(x,y)$-caminhos internamente disjuntos $Q_1, Q_2, \dots, Q_{C_G}$. Denote por $Q_i'$ o caminho resultante de $Q_i$ após a remoção da extremidade $x$. Note que $P_i'$ é um caminho tanto em $G/Y$ como em $G$. Sem perda de generalidade, suponha que cada caminho $Q_i'$ seja um $(s_i,y)$-caminho, para $i \in \{1,\dots,c_G - 1\}$, e suponha que $Q_{c_G}$ seja um $(v,y)$-caminho.
...
\begin{teor}[Teorema de Schur]
  Para todo $k$ existe $n$ (em função de $k$) tal que, se os números
  em $\{1,2,\dots,n\}$ forem particionados em $k$ classes $V_1, \dots, V_k$, então
  existem $x,y \in \{1,\dots,n\}$ tais que $x$, $y$, e $x + y$ todos
  pertencem à mesma classe~$V_i$ para algum~$i$.
\end{teor}

\begin{exer}
  Use o Teorema de Ramsey (versão p/ $k$ cores) para provar o Teorema de Schur. [Dica. Atribua $k$ cores aos pares da forma $xy$ de acordo com a classe que contém o número $|x - y|$. Isto é, a cor de $xy$ é~$i$ se $|x - y| \in V_i$.]
\end{exer}
