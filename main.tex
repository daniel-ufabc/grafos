...
\begin{teor}
\label{teor:menger}
Se~$G$ é um grafo de ordem $n \geq 2$ e $x,y$ são vértices distintos e não adjacentes em~$G$, então
\begin{equation}
...
\begin{proof}
  A prova é por indução em~$m$. Para estabelecermos a base da indução, devemos provar que o teorema vale para todo o grafo vazio com $n \geq 2$ vértices. De fato, em qualquer grafo vazio~$E_n$, se tomarmos dois vértices distintos $x,y$, não há caminho de $x$ a~$y$. Portanto, $p(x,y) = c(x,y) = 0$.

Para provarmos o passo de indução, seja $G$ um grafo qualquer com $m > 0$ arestas, e sejam $x,y$ dois vértices distintos de~$G$. Vamos supor, como hipótese de indução (forte), que qualquer grafo com menos que $m$ arestas satisfaz~\eqref{eq:min_max_conn2}. Seja $\mathcal{P}$ uma coleção de $(x,y)$-caminhos internamente disjuntos em~$G$ com $|\mathcal{P}| = p(x,y)$. 

Pelo Lema~\ref{lema:all_paths_size_2}, podemos supor que existe um vértice $v$ em um dos caminhos de~$\mathcal{P}$ que é vizinho de~$x$ mas não de~$y$. Considere o grafo $G/xv$ como sendo o grafo resultante da contração da aresta $e$ ao vértice $x$. Note que~$\mathcal{P}$ em~$G$ corresponde a uma coleção~$\mathcal{P}/xv$ de $(x,y)$-caminhos internamente disjuntos em~$G/xv$ e que $|\mathcal{P}| = |\mathcal{P}/xv|$. Para facilitar a escrita, vamos usar $c,c',p,p'$ para denotar as quantidades
\begin{align*}
  c &= c_G(x,y),\\
  c' &= c_{G/xv}(x,y),\\
  p &= p_G(x,y),\\
  p' &= p_{G/xv}(x,y).
\end{align*}
Pela hipótese de indução, sabemos que~$G/xv$ satisfaz~\eqref{eq:min_max_conn2}, ou seja, $p' = c'$. , pelo fato de $G'$ ser subgrafo de~$G$, devemos ter $p'\leq p$ e $c'\leq c$. Por outro lado, vamos argumentar que $c'$ não pode ser muito menor que $c$. De fato, se $S$ é um conjunto $(x,y)$-separador em $G-e$, então $S$ mais uma das extremidades da aresta $e$ é $(x,y)$-separador em $G$, portanto, $c' \geq c - 1$. %Com relação à $(u,v)$-conexidade, note que, em qualquer coleção de $(u,v)$-internamente disjuntos em $G$, a aresta $e$ pertence a, no máximo, um desses caminhos, o que nos leva a concluir que $p' \geq p - 1$.
